...
public:
	...
	template<typename T>
	static void write(T& data, std::ofstream& stream) {
		stream.write((char*)&data, sizeof(T));
	}

	static void write(std::string& str, std::ofstream& stream) {
	...
	template<typename T>
	static void write(std::vector<T>& vec, std::ofstream& stream) {
	...
	static void cacheMesh(WavefrontMesh& mesh, std::string path) {
		std::ofstream cachedFile(path, std::ios::binary);
		write(mesh.meshName, cachedFile);
		write(mesh.vertices, cachedFile);
		write(mesh.uvs, cachedFile);
		write(mesh.normals, cachedFile);
		write(mesh.vertexIndices, cachedFile);
		write(mesh.ambientRgb, cachedFile);
		write(mesh.diffuseRgb, cachedFile);
		write(mesh.specularRgb, cachedFile);
		write(mesh.specularStrength, cachedFile);
		write(mesh.illuminationModel, cachedFile);
		write(mesh.reflectivityTexturePath, cachedFile);
		write(mesh.diffuseTexturePath, cachedFile);
		write(mesh.specularTexturePath, cachedFile);
		write(mesh.alphaTexturePath, cachedFile);
		write(mesh.bumpTexturePath, cachedFile);
		write(mesh.materialName, cachedFile);
		cachedFile.close();
	}

	struct WavefrontMeshCacheMetadata {
		size_t numMeshes;
	};

	template<typename T>
	static void read(T* data, std::ifstream& stream) {
		stream.read((char*)data, sizeof(T));
	}

	static  void read(std::string* str, std::ifstream& stream) {
		size_t size;
		read(&size, stream);
	...
	template<typename T>
	static void read(std::vector<T>* vec, std::ifstream& stream) {
		size_t size;
		read(&size, stream);
		vec->reserve(size);
	...
	static void cacheMeshes(std::vector<WavefrontMesh> meshes, std::string path) {
		int i = 0;

		std::ofstream meshMetadata(path + ".cached_mesh_metadata", std::ios::binary);
		auto size = WavefrontMeshCacheMetadata{ meshes.size() };
		meshMetadata.write((char*)&size, sizeof(size_t));
		meshMetadata.close();

		for (WavefrontMesh& mesh : meshes) {
			std::string cachePath = path + ".cached_mesh" + std::to_string(i) + ".bin";
			cacheMesh(mesh, cachePath);
			i++;
		}
	}

	static bool cacheExists(std::string path) {
		std::ifstream meshMetadata(path + ".cached_mesh_metadata", std::ios::binary);
		bool exists = meshMetadata.good();
		meshMetadata.close();
		return exists;
	}
	static void printPos(std::ifstream& ifstream) {
		std::cout << "Position: "<<ifstream.tellg() << "\n";
	}
	static std::vector<WavefrontMesh> loadFromCache(std::string path) {
		std::ifstream meshMetadata(path + ".cached_mesh_metadata", std::ios::binary);
		WavefrontMeshCacheMetadata metadata;
		metadata.numMeshes = 0;
		meshMetadata.read((char*)&metadata, sizeof(WavefrontMeshCacheMetadata));
		meshMetadata.close();
		std::vector<WavefrontMesh> meshes;
		for (int i = 0; i < metadata.numMeshes; i++) {
			std::ifstream cachedFile(path + ".cached_mesh" + std::to_string(i) + ".bin", std::ios::binary);
			WavefrontMesh mesh;
			read(&mesh.meshName, cachedFile);
			read(&mesh.vertices, cachedFile);
			read(&mesh.uvs, cachedFile);
			read(&mesh.normals, cachedFile);
			read(&mesh.vertexIndices, cachedFile);
			read(&mesh.ambientRgb, cachedFile);
			read(&mesh.diffuseRgb, cachedFile);
			read(&mesh.specularRgb, cachedFile);
			read(&mesh.specularStrength, cachedFile);
			read(&mesh.illuminationModel, cachedFile);
			read(&mesh.reflectivityTexturePath, cachedFile);
			read(&mesh.diffuseTexturePath, cachedFile);
			read(&mesh.specularTexturePath, cachedFile);
			read(&mesh.alphaTexturePath, cachedFile);
			read(&mesh.bumpTexturePath, cachedFile);
			read(&mesh.materialName, cachedFile);
			cachedFile.close();
	...
	{
		if (cacheExists(std::string(path))) {
			return loadFromCache(std::string(path));
		}
		...
		cacheMeshes(meshes, std::string(path));
